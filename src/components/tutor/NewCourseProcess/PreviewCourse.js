...
function LessonDetails(id){
  const loadview=()=>{
    ...
    return (
      <LessonButtons key={id.id} onClick={loadview} className="p-3 m-3 text-center"> 
                {!loading && id.title}
                {loading && <Spinner/>}
      </LessonButtons>
      );
    ...
    <div fluid className="">
      ...
      <Container className="height-half mx-sm-0">
        ...
        <Col xs={3}>
            ...
            <AddLessonButtons to={`/tutor-create-lesson/${courses.id}`} className="p-3 m-3 text-center"> 
              + Add Lesson
            </AddLessonButtons>
          </Col>
        ...
        <Col className="text-center">
              ...
              <Button onClick={nextStep} className="primary-button">
                {!courses.verified && <span>Publish</span>}
              </Button>
              </Col>
      </Container>
    </div>
